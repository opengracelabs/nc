"""M36 substrate write path for Cleveland Museum of Art records."""
...
from .rights import CMA_RIGHTS_POLICY_ID
...
def derive_anchor_type(normalized: dict[str, Any], media_type_id: str) -> str:
    """Derive a governed source_item anchor type from CMA metadata."""
    subject_text = " ".join(
    ...
    if any(token in combined for token in BIOLOGICAL_ANCHOR_TOKENS):
        return "biological"
    if normalized.get("find_spot"):
        return "geographic"
    culture = normalized.get("culture")
    if culture and (
        (isinstance(culture, list) and culture)
        or (isinstance(culture, str) and culture)
    ):
        return "geographic"
...
async def write_record(
    ...
    *,
    ...
    """Write one CMA record into M36 tables when CMA Rights Matrix v1 allows it."""
    normalized = normalize_record(raw_payload)
    if normalized.get("rights_decision") == "BLOCKED":
        return {
            "status": "rejected",
            "reason": normalized.get("cma_rights_basis"),
