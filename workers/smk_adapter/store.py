"""M36 substrate write path for Statens Museum for Kunst records."""
...
from .rights import SMK_RIGHTS_POLICY_ID
...
def derive_anchor_type(normalized: dict[str, Any], media_type_id: str) -> str:
    """Derive a governed source_item anchor type from SMK metadata."""
    subject_text = " ".join(
...
async def write_record(
    ...
    *,
    ...
    """Write one SMK record into M36 tables when SMK Rights Matrix v1 allows it."""
    normalized = normalize_record(raw_payload)
    if normalized.get("rights_decision") == "BLOCKED":
        return {
            "status": "rejected",
            "reason": normalized.get("smk_rights_basis"),
