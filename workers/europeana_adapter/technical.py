"""Technical metadata extraction for Europeana substrate intake."""
...
TECHNICAL_SCHEMA_VERSION = "europeana-edm-technical-v1"
VALIDATOR_NAME = "europeana_adapter.technical"
...
def quality_flag(width_px: int | None, height_px: int | None) -> str:
    """Classify the Europeana visual quality baseline from MSC v1.2 Article 29.2(d)."""
...
def build_technical_metadata(normalized: dict[str, Any], *, media_type_id: str) -> dict[str, Any]:
    """Build media_technical_metadata.content for a normalized Europeana record."""
...
def validation_status(content: dict[str, Any]) -> str:
    """Return the worker validation status for current Sprint 3 visual metadata."""
