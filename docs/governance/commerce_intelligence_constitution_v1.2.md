...
| Supersedes | `commerce_intelligence_constitution_v1.1.md` (v1.1.0) |
...
One Director Decision applied. No articles deleted. Affected articles annotated `[Amended v1.2]`.
Articles not listed are unchanged from v1.1.0.

| Decision | Finding | Resolution | Articles Affected |
|---|---|---|---|
| G-3 | `taxon_commercial_tier_score` occupies intra-subscore weight slots (retail: 0.25, publishing: 0.15, reference: 0.20) that are unconditionally zero for geographic-anchored assets. `anchor_weight_spec` from AI v1.1 corrects composite subscore weights but cannot modify intra-subscore signal values — it operates at the outer layer of the formula. The 0.088 suppressed COS weight permanently prevents commercially meritorious maps and geographic illustrations from reaching Tier 1, blocking `museum_print` and `institutional_license` product surfaces. | Add `signal_substitutions` block to `anchor_weight_spec.{anchor_type}`. When present, the scoring worker substitutes the named signals before intra-subscore computation, using the substitute signal's resolved value at the original signal's weight. `substituted_signals_applied` is written to `score_inputs` as a replay input. New activation invariants validate all substitution entries. Replay protocol amended to reconstruct substitutions from stored values. Open Question Q2 closed. | 6, 13, 14, 37, Part X |
...
### Article 6 — Signal Vocabularies `[Amended v1.2]`
...
**G-3 Amendment — Signal Substitutions:** For anchor-type-specific scoring, the active
`commerce_policy.formula_spec` may define a `signal_substitutions` map within
`anchor_weight_spec.{anchor_type}`. When present, the scoring worker applies the substitution
before intra-subscore computation. A substitution redirects an existing intra-subscore signal
value slot to a substitute signal — the substitute's resolved value is used at the original
signal's weight. Substituted signals are not new vocabulary members. The substitute signal must
be a recognized scored signal. See Article 13 for the governing structure, invariants, and
`score_inputs` recording requirements.
...
| `resolution_tier_score` | `image_width_px` → tier map → numeric (see Article 13) | `0.0` if absent |
...
| `identification_confidence` | Subject clarity under ambiguity (0.0–1.0) | Yes | Blocks Publishing subscore above 0.5 |
| `color_profile` | Color process identification when EXIF absent | Yes | No — defaults to `unknown` |

`identification_confidence` measures the certainty of the primary subject identification. For
biological assets: taxon identification certainty. For geographic assets: cartographic attribution
certainty (is this definitively the stated survey map, by the stated cartographer, of the stated
area?). The behavioral semantics — how confident is the primary identification — apply to both
anchor types without amendment.
...
the publication-stage curator approval gate.

**Gate 0 — Rights Record Existence**

```
Condition:    No asset_rights record exists for the linked asset, OR rights_status
              NOT IN ('Public Domain', 'CC0')
Action:       hard_gate_status = 'blocked_rights'
              Audit log: event_type = 'hard_gate_blocked'
```

Note: This gate enforces the existing `check_collection_asset_commercial_ready()` constraint as a
scoring pre-condition.
...
```
Condition:    image_width_px < hard_gate_values.min_image_width_px (default: 2000px)
...
```
Condition:    rights_confidence = 0.0 (legal hold indicator)
Action:       hard_gate_status = 'blocked_legal'
              Audit log: event_type = 'hard_gate_blocked'
              Not overridable by curator. Requires source-level rights resolution.
```

**Gate 4 — Quality**

```
Condition:    image_quality_score < hard_gate_values.min_quality_score (default: 0.40)
              OR image_quality_score IS NULL
Action:       COS is computed (to prioritize the review queue).
              If score >= 0.40: trigger 'signal_correction' recompute; gate lifts.
              If score < 0.40:  hard_gate_status = 'blocked_quality' permanent.
```

Note: Gate 4 is post-scoring. COS is computed to prioritize the human review queue. Gate 4 blocks
routing, not scoring.

**Gate 5 — Curator Approval (Publication Stage)**

```
Condition:    commerce_opportunities.curator_decision != 'approved'
              at the time a product_recommendation or collection_recommendation
              attempts to advance to status = 'assigned'
Location:     This gate lives in product_recommendations and collection_recommendations,
              not in commerce_opportunities.
```
...
results are not independently logged — Gate 3 is the result of record.
...
record update). When a cure condition is satisfied, the hard gate status is lifted and a
`hard_gate_passed` event is written.

A curator may manually escalate a gate result only when the automated cure path is unavailable.
Curator escalation writes a `curator_reviewed` event with the curator's identity and rationale. It
does not change the scoring algorithm — it only changes the gate status.
...
The replayability guarantee is: given only the data in PostgreSQL at the time of scoring, any
historical COS and CSM score can be reproduced exactly. The guarantee is violated if:

1. Any signal value in `score_inputs` is a live reference (a re-query) rather than a resolved
   concrete value at scoring time.
2. Any formula weight, threshold, or algorithm parameter is hardcoded in worker code rather than
   read from `commerce_policy.formula_spec`.
3. Any `score_audit_log` entry is modified or deleted after creation.
4. Any substituted signal value in `signal_substitutions_applied` (Article 13) is derived by
   re-querying the substitute signal at replay time rather than using the stored value.

The guarantee applies to both Pass 1 (COS) and Pass 2 (CSM). A `replay_failure` on either pass
is a constitutional breach.

### Article 13 — The formula_spec Contract `[Amended v1.2]`
...
scoring weights, thresholds, formula structure, or signal substitution rules may be hardcoded in
worker code.
...
**Two-pass scoring algorithm: `[Amended v1.2]`**
...
Pass 1 — COS:
  ...
  2a. Signal substitution [G-3 — v1.2]:
      Read anchor_type from illustration_opportunities (via linked commerce_opportunity).
      If anchor_weight_spec.{anchor_type}.signal_substitutions is present in formula_spec:
        For each (original_signal → substitute_signal) pair in signal_substitutions:
          a. Resolve substitute_signal's current scored value (must already be present
             in the resolved signal set from step 2 — no additional query).
          b. Record in score_inputs:
               "signal_substitutions_applied": {
                 "<original_signal>": {
                   "substituted_by":   "<substitute_signal>",
                   "substitute_value": <float — 6 decimal places>
                 }
               }
          c. In all subsequent intra-subscore computations, replace every occurrence of
             original_signal's value with substitute_value. The weight is unchanged.
          d. Null policy: if substitute_signal's resolved value is null, apply
             null_signal_policy as if original_signal were null.
      Signal substitution applies to Pass 1 (COS) intra-subscore computation only.
      Pass 2 (CSM) dimensions are not subject to signal substitution.

  3. Apply anchor_weight_spec.{anchor_type}.composite_modifier to determine per-subscore
     weights for the composite formula. If anchor_type has no composite_modifier entry,
     use formula_spec.composite weights unchanged.

  4. Apply formula_spec.subscores (with substitutions from step 2a) to compute
     museum_score, retail_score, publishing_score, tourism_score, reference_score.

  5. Apply the composite_modifier-adjusted weights to compute
     commerce_opportunity_score (COS).

  6. Derive commerce_tier from COS and tier_thresholds.

  7. Evaluate product eligibility flags.

  8. Write score_inputs and score_outputs (Pass 1 portion) to score_audit_log.
     score_inputs must include signal_substitutions_applied if step 2a was executed.

Pass 2 — CSM:
  9. Using the same score_inputs from step 2 (no re-fetch):
  10. Apply csm_dimension_map.dimensions to compute VAS, PIS, SSS, TAS, IPS, PVS.
  11. Apply csm_dimension_map.composite to compute csm_score.
  12. Derive csm_tier from csm_score and csm_dimension_map.tier_thresholds.
  13. Append csm_score and csm_tier to score_outputs in the same score_audit_log entry.
...
**`anchor_weight_spec` — Governing Rules: `[G-3 — v1.2]`**

**Composite modifier:** When `anchor_weight_spec.{anchor_type}.composite_modifier` is present,
the scoring worker uses its per-subscore weights in the composite formula instead of
`formula_spec.composite` defaults. Composite modifier weights must sum to exactly `1.00`. This is
validated at policy activation time.

**Signal substitutions:** When `anchor_weight_spec.{anchor_type}.signal_substitutions` is
present, each entry is a `{ original_signal: substitute_signal }` pair. The substitution:

- Replaces the original signal's *value* in intra-subscore computation. The weight slot is
  unchanged.
- Applies to every subscore in `formula_spec.subscores` that contains the original signal as
  an input. It does not apply to `csm_dimension_map.dimensions`.
- Uses the substitute signal's resolved value as of step 2 signal resolution. No additional
  query is performed.
- Does not alter the formula weight sums — the substitution is a value replacement, not a
  structural change.

**`score_inputs` recording:** `signal_substitutions_applied` is written as a JSONB field in
`score_inputs` whenever step 2a executes. It is a replay input — see Article 14.
...
- The composite_modifier weights within each `anchor_weight_spec.{anchor_type}` block must sum
  to exactly `1.00`.
...
- `blend_algorithm_version` in `anchor_weight_spec.mixed` must match a registered algorithm in
  the scoring worker.
- **Signal substitution invariants [G-3 — v1.2]:** For each entry in any
  `anchor_weight_spec.{anchor_type}.signal_substitutions`:
  1. The original signal name must appear as a `signal` key in at least one input array within
     `formula_spec.subscores`.
  2. The substitute signal name must be a recognized scored signal (Class A or Class B) whose
     value is available at the time intra-subscore computation executes.
  3. The substitute signal name must not equal the original signal name.
  4. The substitute signal name must not itself appear as an original signal name in the same
     `signal_substitutions` block (no substitution chains).
- A policy is rejected at the `pending_approval → active` gate if any invariant fails.

### Article 14 — The Replay Protocol `[Amended v1.2]`
...
2a. Signal substitution reconstruction [G-3 — v1.2]:
    If entry_N.score_inputs contains "signal_substitutions_applied":
      The values in signal_substitutions_applied are replay inputs — they were resolved
      and recorded at original scoring time (step 2a of the scoring algorithm).
      For each (original_signal → { substituted_by, substitute_value }) entry:
        Use substitute_value as the value for original_signal in intra-subscore computation.
        Do NOT re-query the substitute signal from PostgreSQL.
        Do NOT load the signal_substitutions map from the policy and re-derive the value.
        The stored substitute_value is the authoritative replay input.
      If entry_N.score_inputs does NOT contain "signal_substitutions_applied" but the
      loaded policy does contain signal_substitutions for the opportunity's anchor_type:
        This is a replay_failure. Signal substitution was expected but not recorded.
        Write replay_failure event and flag opportunity as integrity_suspect.

3a. Apply formula_spec.scorer_version algorithm (Pass 1) to entry_N.score_inputs,
    using substituted values from step 2a where applicable.
...
`signal_substitutions_applied`, when present in `score_inputs`, is subject to the same canonical
JSON serialization rules and is included in `entry_checksum_sha256` computation.
...
| Composite weight change, model weight change, signal substitution rule change | Minor (1.0.0 → 1.1.0) | Second-human approval required |
...
Adding or modifying a `signal_substitutions` entry in `anchor_weight_spec` is a **Minor** version
bump. It changes scoring outputs for the affected anchor type and requires second-human approval.
...
When a Minor version bump that adds or modifies `signal_substitutions` is activated, recompute
is required only for opportunities whose `anchor_type` matches the amended anchor type. The
recompute worker must filter `commerce_opportunities` by the linked opportunity's `anchor_type`
to avoid unnecessary full-table recompute.
...
`tourism_score`, `reference_score`) are the authoritative runtime columns.
...
```json
{
  ...
  "signal_substitutions_applied": {
    "taxon_commercial_tier_score": {
      "substituted_by": "place_relevance_score",
      "substitute_value": 0.900000
    }
  }
}
```

`signal_substitutions_applied` is required in `recommendation_basis` when signal substitution
was applied during scoring. An auditor reading `recommendation_basis` must be able to understand
why the signal substitution occurred and what value was used, without access to any other system.
...
| What | `event_type` + `score_inputs` + `score_outputs` | Mandatory; full snapshot required; `score_outputs` includes both COS and CSM; `score_inputs` includes `signal_substitutions_applied` when applicable |
...
2. **PostgreSQL RULE (no DELETE):** Any DELETE statement raises an exception at the database
   level.
3. **Entry checksum:** `entry_checksum_sha256` covers the full record content including
   `signal_substitutions_applied` when present. Modification invalidates the checksum.
...
|---|---|
| Transparency | `commerce_policy.formula_spec` is the transparency artifact. Human-readable. Stored in PostgreSQL. Both Pass 1 (COS) and Pass 2 (CSM) algorithms are fully specified. `anchor_weight_spec.signal_substitutions` is part of this artifact — anchor-type-specific scoring rules are governed and transparent. |
| Explainability | `recommendation_basis` on `product_recommendations` is the per-decision explainability artifact. `signal_substitutions_applied` within `recommendation_basis` makes signal substitution visible to auditors. |
...
| Accuracy and Reliability | The replay protocol (Article 14) is the accuracy verification mechanism. Both passes are verified. Signal substitution is verified by replaying from stored `substitute_value` in `score_inputs`. |
| Bias Mitigation | COS must not correlate with GBIF occurrence frequency, species popularity, or taxonomic inventory completeness. Any such correlation in a replay analysis indicates a contaminated signal and is a constitutional breach. Signal substitution for geographic assets uses `place_relevance_score` — a place-commercial-significance signal with no biological content. |
...
## Part X — Prohibited Acts `[Amended v1.2]`
...
8. **Hardcoding formula weights, tier thresholds, hard gate values, or signal substitution rules
   in worker code.** All scoring parameters — including `anchor_weight_spec`,
   `composite_modifier`, and `signal_substitutions` — are sourced exclusively from
   `commerce_policy.formula_spec`.
...
12. **Applying a signal substitution from `anchor_weight_spec.{anchor_type}.signal_substitutions`
    without recording `signal_substitutions_applied` in `score_inputs`. `[G-3 — v1.2]`** The
    substitute value used at scoring time is a replay input. Any scoring event that applies a
    substitution without recording it produces an unreplayable audit entry, which is a
    constitutional breach.

13. **Performing signal substitution at replay time by re-querying the substitute signal from
    PostgreSQL. `[G-3 — v1.2]`** The replay worker must use `score_inputs
    .signal_substitutions_applied.{signal}.substitute_value`. Re-querying produces a
    non-deterministic replay and violates the replayability guarantee.
...
Migration 19b (was Migration 19):
  Create governed vocabulary tables:
    ...
    - place_iconic_taxa_vocabulary
  Seed initial rows (status = 'proposed'; requires curator approval to activate).

Migration 20:
  Create commerce_policy table with 6-state status vocabulary.
  Add max_score_age_days column.
...
Migration 21:
  ...
  Five model subscores, csm_score, csm_tier, last_scored_at.
  Constraint trigger: hard_gate_status != 'passed' → eligibility flags = FALSE.
  Constraint trigger: COS present → audit log entry required.
...
Migrations M-32 through M-35 (Asset Intelligence v1.1):
  Governed by Asset Intelligence Constitution v1.1.
  M-32: anchor_type field on illustration_opportunities.
  M-33: creator_prestige_registry, creator_authority_registry, registry_audit_log.
  M-34: place_iconic_taxa_registry.
  M-35: Activation — anchor_weight_spec added to commerce_policy.formula_spec.
        (Minor policy version bump; second-human approval required.)
        anchor_weight_spec must include:
          - cross_anchor_discount_rate
          - composite_modifier per anchor type
          - signal_substitutions per anchor type where applicable
          - blend_algorithm_version for mixed anchor type
...
Activation (governed event, not a migration):
  ...
  Step 4: Scoring worker activated (with signal substitution support per Article 13).
  Step 5: Replay worker activated simultaneously (with substitution replay per Article 14).
```

### Article 37 — Open Architectural Questions `[Amended v1.2]`

The following questions were open in v1.0.0. Resolved questions are recorded here.
...
`CHECK (source IN ('bhl', 'loc'))` before Migration 22.

**[RESOLVED — v1.1] Q3 — `place_iconic_taxa` governance process.**
Decision D-3/D-5 resolved this question. `place_iconic_taxa_vocabulary` is a governed table.

**[RESOLVED — v1.2] Q2 — `anchor_type` on `illustration_opportunities` and scoring formula
gap for geographic assets.**

Two-part resolution:

Part 1 (Asset Intelligence Constitution v1.1, M-32): `anchor_type` is added to
`illustration_opportunities` as a governed field. LOC opportunities may be scored once `anchor_type`
is set; its absence triggers `null_signal_policy = "null_blocks_tier_12_advancement"` until M-32
is applied. M-32 backfills all existing records as `'biological'`.

Part 2 (this amendment — CI v1.2, Decision G-3): `signal_substitutions` is added to
`anchor_weight_spec.geographic`. For geographic-anchored assets, `place_relevance_score`
substitutes for `taxon_commercial_tier_score` in `retail_score`, `publishing_score`, and
`reference_score` intra-subscore computation. This resolves the 0.088 suppressed COS weight
identified in the Historic Maps Activation Plan v1. Strong geographic assets (Tier 1 cartographer
+ iconic place + seeded place-iconic registry + curator-reviewed Class C signals) can now achieve
COS ≥ 0.80 and qualify for `commerce_tier = 'tier_1'`, accessing `museum_print` and
`institutional_license` product surfaces.
...
No API endpoint or admin workflow for Class C signal submission is defined. The
`null_signal_policy = "null_blocks_tier_12_advancement"` rule gates tier advancement without a
review workflow, but curators have no mechanism to submit `image_quality_score` or
`composition_fit` values. This must be resolved before the first Tier 1 or Tier 2 classification
is needed.
...
|---|---|---|
| Staleness window | `max_score_age_days` | 90 days |

Changes to `max_score_age_days` are a Minor version bump. They do not require a full recompute —
only the threshold changes, not the scoring formula.
